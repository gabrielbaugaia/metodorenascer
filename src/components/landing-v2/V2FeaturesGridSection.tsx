...
const features = [
  ...
  { ico: "🔋", title: "Prontidão", tag: "GabrielBau Score™", desc: "Determina o volume exato de treino para o dia — 100%, 80%, 50% ou recuperação ativa — com base em como você acordou." },
    ...
    <section ref={ref} className="py-[110px] px-7 md:px-[60px] bg-card/50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[2px]">
        {features.map((f, i) => (
          <div
            key={f.title}
            className={`bg-card border border-border p-10 relative overflow-hidden group hover:border-foreground transition-all duration-300 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}
            style={{ transitionDelay: `${i * 80}ms` }}
          >
            {/* Left accent bar */}
            <div className="absolute left-0 top-0 w-[3px] h-0 bg-foreground group-hover:h-full transition-all duration-350" />
            {/* Corner fold */}
            ...
            <span className="font-mono-v2 text-[9px] tracking-[2.5px] uppercase text-foreground block mb-3.5">{f.tag}</span>
