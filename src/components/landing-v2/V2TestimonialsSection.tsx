  ...
  return (
    <section ref={ref} className="py-[110px] px-7 md:px-[60px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-[2px]">
        {testimonials.map((t) => (
          <div key={t.name} className="bg-card border border-border p-9 relative hover:border-foreground transition-colors">
            <span className="absolute top-6 right-7 font-display-v2 text-[60px] text-foreground/7 leading-none">"</span>
            <div className="text-foreground text-[14px] tracking-[3px] mb-4">{t.stars}</div>
            <p className="font-body-v2 text-[14px] text-foreground/80 leading-[1.75] font-light italic mb-6">{t.text}</p>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-foreground/8 border border-foreground flex items-center justify-center font-display-v2 text-[18px] text-foreground">
                {t.initial}
              </div>
              <div>
