...
const plans = [{
  ...
  features: ["Treino personalizado", "Receitas exclusivas", "Fale com Mentor 24h", "Dashboard de progresso", "Análise de fotos", "Acesso vitalício ao preço"],
  popular: true,
  promotional: true
}, {
  name: "MENSAL",
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto text-lg mb-2">
            De R$197/mês por apenas R$49/mês os embaixadores terão preço vitalício sem acréscimos eternamente. Apenas para os primeiros 25 clientes cadastrados. Não perca essa OPORTUNIDADE ÚNICA.
          </p>
        ...
        <div className="grid md:grid-cols-3 lg:grid-cols-5 gap-6 max-w-7xl mx-auto mt-12">
          {plans.map((plan, index) => (
            <Card 
              key={plan.name} 
              className={`relative overflow-hidden animate-fade-in transition-all duration-300 ${plan.popular ? 'bg-primary/10 border-primary/50 md:scale-105' : 'bg-card border-border/50'}`} 
