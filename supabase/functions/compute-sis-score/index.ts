...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Aggregate daily volumes from workout_set_logs
    ...
    const sleepVals = dayLogs.filter(l => l.sleep_hours).map(l => l.sleep_hours!);
    const stressVals = dayLogs.filter(l => l.stress_level).map(l => l.stress_level!);
    ...
    if (todayDayLog || todayHealth) {
      ...
      const fatigueComp = map15(energyLvl); // energy as proxy for inverse fatigue
    ...
    // ======= STRUCTURAL SCORE =======
    let structuralScore = 50; // fallback neutral
    ...
    return createSuccessResponse(req, scoreRow);
  } catch (err) {
